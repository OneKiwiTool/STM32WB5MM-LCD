...
#if 0
void ST7789_WriteChar(uint16_t x, uint16_t y, char ch, sFONT font, uint16_t color, uint16_t bgcolor)
{
	uint32_t i, b, j;
	ST7789_Select();
...
void ST7789_WriteChar(uint16_t Xpoint, uint16_t Ypoint, const char Acsii_Char,
                    sFONT* Font, uint16_t Color_Background, uint16_t Color_Foreground)
{
    uint16_t Page, Column;

    if (Xpoint > ST7789_WIDTH || Ypoint > ST7789_HEIGHT) {
        ...
        for (Column = 0; Column < Font->Width; Column ++ ) {
            ...
            if (FONT_BACKGROUND == Color_Background) { //this process is to speed up the scan
                if (*ptr & (0x80 >> (Column % 8)))
					...
					ST7789_DrawPixel(Xpoint + Column, Ypoint + Page, Color_Foreground);
                ...
                if (*ptr & (0x80 >> (Column % 8))) {
					...
					ST7789_DrawPixel(Xpoint + Column, Ypoint + Page, Color_Background);
...
void ST7789_WriteString(uint16_t Xstart, uint16_t Ystart, const char * pString,
                         sFONT* Font, uint16_t Color_Background, uint16_t Color_Foreground )
{
    uint16_t Xpoint = Xstart;
    uint16_t Ypoint = Ystart;

    if (Xstart > ST7789_WIDTH || Ystart > ST7789_HEIGHT) {
    ...
    while (* pString != '\0') {
        ...
        if ((Xpoint + Font->Width ) > ST7789_WIDTH ) {
            Xpoint = Xstart;
            Ypoint += Font->Height;
        }

        // If the Y direction is full, reposition to(Xstart, Ystart)
        if ((Ypoint  + Font->Height ) > ST7789_HEIGHT ) {
            Xpoint = Xstart;
            Ypoint = Ystart;
        }
        ST7789_WriteChar(Xpoint, Ypoint, * pString, Font, Color_Background, Color_Foreground);
...
#if 0
void ST7789_WriteString(uint16_t x, uint16_t y, const char *str, sFONT font, uint16_t color, uint16_t bgcolor)
{
	ST7789_Select();
	while (*str) {
...
void ST7789_Test(void)
{
	...
	ST7789_DrawFilledRectangle(30, 30, 50, 50, BLUE);
	HAL_Delay(1000);
